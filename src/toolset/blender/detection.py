...
def install_kotorblender(info: BlenderInfo) -> bool:
    """Install kotorblender from vendor directory to Blender addons/extensions directory.
    ...
    kotorblender_path = info.kotorblender_path
    if kotorblender_path is None:
        RobustLogger().error("Cannot determine kotorblender installation path")
        return False

    # Find vendor kotorblender directory
    # Try to find repo root by looking for vendor/kotorblender
    ...
    for parent in current_file.parents:
        vendor_kotorblender = parent / "vendor" / "kotorblender" / "io_scene_kotor"
        if vendor_kotorblender.exists() and (vendor_kotorblender / "__init__.py").exists():
            repo_root = parent
            break
    
    if repo_root is None:
        RobustLogger().error("Could not find vendor/kotorblender directory")
        return False

    vendor_kotorblender = repo_root / "vendor" / "kotorblender" / "io_scene_kotor"
    if not vendor_kotorblender.exists():
        RobustLogger().error(f"kotorblender source not found at {vendor_kotorblender}")
        return False

    try:
        # Create parent directory if needed
        kotorblender_path.parent.mkdir(parents=True, exist_ok=True)

        # Remove existing installation if present
        if kotorblender_path.exists():
            shutil.rmtree(kotorblender_path)

        # Copy kotorblender from vendor to Blender directory
        shutil.copytree(vendor_kotorblender, kotorblender_path)

        RobustLogger().info(f"Installed kotorblender to {kotorblender_path}")

        # Re-check installation
        info.has_kotorblender = _check_kotorblender_installed(info)
        return info.has_kotorblender

    except OSError as e:
        RobustLogger().error(f"Failed to install kotorblender: {e}")
        return False
