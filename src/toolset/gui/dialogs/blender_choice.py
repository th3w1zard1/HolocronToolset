...
# DeadlyStream download URL for kotorblender
KOTORBLENDER_DOWNLOAD_URL = "https://deadlystream.com/files/file/1853-kotorblender/"
...
class BlenderChoiceDialog(QDialog):
    ...
    def _setup_ui(self):
        ...
        if self._blender_info.is_valid:
            ...
            if self._blender_info.has_kotorblender:
                ...
                install_hint = QLabel(
                    "<small>kotorblender is required to use Blender. "
                    "Install it in Blender to enable the Blender option.</small>"
                )
                ...
                download_button = QPushButton("Download kotorblender")
                download_button.setToolTip(
                    f"Opens {KOTORBLENDER_DOWNLOAD_URL}\n"
                    "Download and install the addon in Blender, then restart this dialog."
                )
                download_button.clicked.connect(self._open_download_link)
                download_button_layout.addWidget(download_button)
                download_button_layout.addStretch()
                info_layout.addLayout(download_button_layout)
        ...
        # Disable button if kotorblender is missing (required)
        ...
        if self._blender_info.has_kotorblender:
            ...
            tooltip = (
                ...
                "Download and install kotorblender in Blender, then reopen this dialog."
            )
    ...
    def _open_download_link(self):
        """Open the kotorblender download page in browser."""
        from qtpy.QtCore import QUrl
        QDesktopServices.openUrl(QUrl(KOTORBLENDER_DOWNLOAD_URL))
